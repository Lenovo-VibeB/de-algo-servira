...
ifneq ($(strip $(MTK_BASIC_PACKAGE)), yes)
    ifneq ($(strip $(MTK_BSP_PACKAGE)), yes)
        ifeq ($(strip $(HQ_USE_GOOGLE_CALENDAR)), yes)
...
ifeq ($(strip $(HQ_BICR_SUPPORT_CMCC)), yes)
PRODUCT_COPY_FILES += device/mediatek/common/Lenovo_CdRom_CMCC/iAmCdRom.iso:system/etc/iAmCdRom.iso
else ifeq ($(strip $(HQ_BICR_SUPPORT_ROW)), yes)
PRODUCT_COPY_FILES += device/mediatek/common/Lenovo_CdRom_ROW/iAmCdRom.iso:system/etc/iAmCdRom.iso
else ifeq ($(strip $(HQ_BICR_SUPPORT_ROW_AL732)), yes)
PRODUCT_COPY_FILES += device/mediatek/common/Lenovo_CdRom_ROW_AL732/iAmCdRom.iso:system/etc/iAmCdRom.iso
else ifeq ($(strip $(HQ_BICR_SUPPORT_ROW_AL910AP)), yes)
PRODUCT_COPY_FILES += device/mediatek/common/Lenovo_CdRom_ROW_AL910AP/iAmCdRom.iso:system/etc/iAmCdRom.iso
else
...
# wangyi add for country code adaptation begin
PRODUCT_PACKAGES += \
    nvram_country \
    factoryMain \
    factoryMain2

ifeq ($(strip $(HQ_APP_HIDE)), yes)
   PRODUCT_PACKAGES += AppHide
endif

ifeq ($(strip $(HQ_SHOW_COUNTRY_LIST_FOR_ROW)), yes)
    PRODUCT_PACKAGES += CountryList
endif
# wangyi add for country code adaptation end

#wangyi add start
ifeq ($(strip $(HQ_SHOW_COUNTRY_LIST_FOR_ROW)),yes)
  PRODUCT_PROPERTY_OVERRIDES += ro.sys.countrylist.start=1
endif
#wangyi add end
...
-include huaqin/config/common/device.mk
#end

########################################################################
#                                                                      #
# DONT MODIFY THIS FILE,PLEASE MODIFY huaqin/config/common/device.mk  #
#                                                                      #
########################################################################
